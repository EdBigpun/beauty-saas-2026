...
interface Cita {
  ...
  clientName: string;
  clientPhone: string;
  clientNotes: string; // ¡NUEVO! Agregamos este campo para leer el servicio
  startTime: string;
  status: string;
}
  ...
  const handleConfirm = async (id: number) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:9090/api/appointments/${id}/confirm`,
        {
          method: "PUT",
        },
      );
        ...
        {citas.map((cita) => (
          ...
          >
            ...
            <div>
              ...
              {/* (2) MOSTRAR EL SERVICIO Y PRECIO */}
              {/* Si hay notas (info del servicio), las mostramos en color amarillo suave */}
              {cita.clientNotes && (
